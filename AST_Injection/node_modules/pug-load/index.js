...
module.exports = load;
function load(ast, options) { // sau khi được tạo ast sẽ đưa vào load
...
load.string = function loadString(src, options) { // src vẫn là string chúng ta truyền vào và các options vẫn là các options ban đầu
  options = assign(getOptions(options), {
    src: src,
  });
  var tokens = options.lex(src, options); // phân tích từ vựng
  var ast = options.parse(tokens, options); // phân tích cú pháp
  return load(ast, options); // gọi tới load
};
...
load.validateOptions = function validateOptions(options) { // vẫn là check validate
  /* istanbul ignore if */
...
function getOptions(options) { // check validate các options, sao chép các giá trị resolve và read vào trong options
